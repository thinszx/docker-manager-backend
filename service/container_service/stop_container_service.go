...
// StopContainerService 优雅终止容器的服务
type StopContainerService struct {
}

// StopContainer 优雅终止容器
func (service *StopContainerService) StopContainer(containerID string, timeout string) serializer.Response {
	...
	if err = cli.ContainerStop(context.Background(), containerID, &timeoutDuration); err != nil {
		return serializer.Response{
			Status: 500,
			Msg:    fmt.Sprintf("Failed to stop the container \"%s\"", containerModel.Name),
			Error:  err.Error(),
		}
	}

	// 更改container的数据并存储
	containerModel.Status = "Stopped"
